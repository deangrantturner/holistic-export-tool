...
def clean_sku(val):
    ...
    # Remove .0 if it exists (e.g. "10121.0" -> "10121")
...
def show_backup_prompt(key_suffix):
    """Shows a prominent warning to download the backup."""
    if os.path.exists("invoices.db"):
        st.info("✅ **Changes Saved to Database!** Now download the backup to keep them safe.")
        with open("invoices.db", "rb") as f:
            st.download_button(
                "📥 DOWNLOAD UPDATED BACKUP FILE",
                data=f,
...
if os.path.exists("invoices.db"):
    ...
    with open("invoices.db", "rb") as f:
        st.sidebar.download_button(
            ...
            key="sidebar_backup_dl_btn"
        )

st.sidebar.markdown("---")
st.sidebar.info("If the app reset, upload your last backup file here:")
    ...
    if uploaded_file:
        try:
            ...
            if 'Ship to country' not in df.columns:
                ...
                # Use clean_sku here for CSV data
                ...
                if not catalog.empty:
                    # Clean catalog SKUs for matching
                    catalog['sku'] = catalog['sku'].apply(clean_sku)
                ...
                with col_right:
                    ...
                    with st.expander("⚙️ Sender Settings", expanded=True):
                        ...
                        if st.button("💾 Save Credentials"):
                            ...
                            st.success("Saved!")
                            show_backup_prompt("email_creds") # PROMPT HERE
