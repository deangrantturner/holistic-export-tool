...
def show_backup_prompt(key_suffix):
    if os.path.exists("invoices.db"):
        with open("invoices.db", "rb") as f:
            st.warning("⚠️ **Change Saved!** Download backup if you plan to close the tab.")
            st.download_button("📥 Save Backup", data=f, file_name=f"backup_{datetime.now().strftime('%Y%m%d')}.db", mime="application/x-sqlite3", key=f"bk_{key_suffix}")
                ...
                if uploaded_file:
                    ...
                    if 'Ship to country' in df.columns:
                        ...
                        if not cat_check.empty:
                            cat_check['sku'] = cat_check['sku'].astype(str).str.strip()
